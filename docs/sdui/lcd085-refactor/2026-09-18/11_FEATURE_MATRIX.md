...
**T1–T16 的权威清单独立维护于 `docs/sdui/terminal/TERMINAL_CONFIRMATIONS.md`**（2026-09-20 建立）。逐项详情——问终端什么、平台侧当前假设（附代码位置）、假设不成立时的改动代价、确认结果回填位——都在那里。本节只保留状态摘要，不再维护副本。

| 批次 | 主题 | 编号 | 状态 |
| --- | --- | --- | --- |
| A | 连接层（最小可联通集） | T1、T2、T3、T5 | 待确认 |
| B | 二进制载荷 | T7、T8、T12 | 待确认 |
| C | 业务配置与交互 | T4、T13、T14、T15 | 待确认 |
| D | 音频链路 | T9、T11、T16 | 待确认 |
| E | 独立项 | T6、T10 | 待确认 |

**最小可联通集是批次 A**：这四项确认后平台侧即无阻塞，终端可开始 v2 联调，不必等 16 项全部定稿。分阶段推进表见独立清单 §4。

| 编号 | 状态 |
| --- | --- |
| ~~T17~~ | **已关闭（0.14.2）**：`ui` 包解耦的入口不是三个 ui 服务，而是**两条边**（`ui.SduiUiTemplateService → DeviceCapabilityProjection`、`section.SectionTypeCatalog → EventRegistry`），均已切断，保留闭包 39 → 22。见 12_DESIGN_NOTES.md §7.3.2 / §4.22 |
