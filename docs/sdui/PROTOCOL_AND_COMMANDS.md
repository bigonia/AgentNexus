...
|----------|------|------|---------|------|
| 9  | `EVENT_INPUT`    | ↑ 上行 | TLV: event_kind, node_id, event_type, ts | UI 交互事件、物理按键 |
...
| 值 | 说明 | 适用板卡 |
|----|------|---------|
| `"touch"` | 触屏交互，无平台可用的实体按键 | AMOLED_175 |
...
| 名称 | 含义 | 出现条件 | 详细定义 |
|------|------|---------|----------|
| `buttons.boot` | BOOT 按键事件 | `input_mode: "keys"` | §4.1 |
...
**能力名称**: `buttons.boot` / `buttons.plus` |
**出现条件**: `input_mode: "keys"`（仅 LCD_085） |
...
**传输**: JSON Topic `motion`

IMU 姿态事件，3 种类型各有冷却期防抖。
...
```json
{
  "topic": "motion",
  "device_id": "esp32-A1B2C3D4E5F6",
  "payload": { "type": "shake", "magnitude": 16.2 }
}
```
...
|------|------|------|
| `type` | string | `"shake"` |
| `magnitude` | float | 加速度幅值（m/s²），阈值 > 14.7 |

冷却期: ~1 秒（10 tick × 100ms）
...
```json
{
  "topic": "motion",
  "device_id": "esp32-A1B2C3D4E5F6",
  "payload": { "type": "wrist_raise", "tilt": 44.0, "delta": 13.5 }
}
```

| 字段 | 类型 | 说明 |
|------|------|------|
| `type` | string | `"wrist_raise"` |
| `tilt` | float | 当前倾斜角度（度），阈值 > 40° |
| `delta` | float | 角度变化量（度），阈值 > 12° |

触发条件: 前 18 tick 内手臂接近水平（tilt < 12°），随后快速倾斜超过 40°。冷却期 ~1.5 秒（15 tick）。
...
```json
{
  "topic": "motion",
  "device_id": "esp32-A1B2C3D4E5F6",
  "payload": { "type": "flip", "from": "face_up", "to": "face_down", "tilt": 7.4 }
}
```
...
|------|------|------|
| `type` | string | `"flip"` |
| `from` | string | 翻转前朝向: `"face_up"` / `"face_down"` |
| `to` | string | 翻转后朝向 |
| `tilt` | float | 当前倾斜角度（度） |
...
| `battery_mv` | int | 电池电压（mV），不支持时为 `-1` |
| `battery_pct` | int | 电量百分比（0–100），不支持时为 `-1` |
...
终端 (ESP32-S3)                          服务器 (AgentNexus)
     ...
     |                                         |
     |--- WS TEXT: motion ------------------->|  ⑥ IMU 事件 (AMOLED_175)
...
| 模块 | 注册函数 | 注册内容 |
|------|----------|----------|
| `main.c` | `system_register_capabilities()` | `display.brightness`(O), `device.reboot`(O), `buttons.*`(I, keys 模式) |
