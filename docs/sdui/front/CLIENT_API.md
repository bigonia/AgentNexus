...
本节端点的调用方是 `static/sdui-v2-console.html`（2026-09-20 新增）：能力页签读 §2.2 的四个端点，请求下发页签用 `/requests` + `/request` + `/requests/history`，主视图页签用 `/view` + `/view/state`，实时流页签订阅两条 SSE。`node-tests` 三个端点仍由旧节点测试页使用，随 P5c 一并处置（见 12_DESIGN_NOTES.md §4.23）。
