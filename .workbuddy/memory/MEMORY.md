...
- 终端待确认项（T1–T16）**唯一维护位置是 `docs/sdui/terminal/TERMINAL_CONFIRMATIONS.md`**，其余文档只做引用。每项固定五段：问终端什么 / 对应缺口 / 平台当前假设（须从代码取证并附类名）/ 阻塞面 / 改动代价与确认回填位。新增措辞用"平台当前假设"而非"平台实现"——它是待核对的临时取值，不是已定稿行为。
- 待确认项按**耦合**分批次确认，不按重要性：同批内任一项不一致会让整批无法联调。最小可联通集是连接层（握手 / hash / 帧头 / 错误名），这几项定完终端即可开工。
- 判断某待确认项是"平台假设待核对"还是"缺输入"：若平台已有实现取值 → 待核对；若平台根本没实现（如五类 Section 字段、Canvas 上限、本地响应动作名全集）→ 缺输入，需终端提供定义，不能问成"你们是不是也这样"。
